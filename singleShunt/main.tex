...
Table~\ref{tab:topologies} summarizes four common shunt placements used with three-phase inverters \cite{Krause2013,Bose2002,Kester2005}.
...
\]
and $\dc_{\max,\mathrm{SS}}\approx 0.94$. The existence bound \eqref{eq:m_min_ss} gives $\midx_{\min,\mathrm{SS}}=0.48$, so unmodified PWM is blind for $\midx<0.48$ \emph{at sector centers} and always near boundaries. If the application needs $\midx>0.88$ with full coverage, the designer must reduce $\treq$ (faster settling), lower $f_{sw}$, or accept blind-zone estimation.
...
\subsection{Where $\treq$ comes from}
