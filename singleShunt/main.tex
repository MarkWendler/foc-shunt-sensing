...
\]
and $\dc_{\max,\mathrm{SS}}\approx 0.94$. The existence bound \eqref{eq:m_min_ss} gives $\midx_{\min,\mathrm{SS}}=0.48$, so unmodified PWM is blind for $\midx<0.48$ \emph{at sector centers} and always near boundaries. If the application needs $\midx>0.88$ with full coverage, the designer must reduce $\treq$ (faster settling), lower $f_{sw}$, or accept blind-zone estimation. An interactive web calculator that implements \eqref{eq:m_max_ss_main}, the blind-zone map \eqref{eq:alpha_blind}, and the SVM duty waveforms is available online \cite{Wendler_calc_2026}, letting readers explore these trade-offs for their own timing budgets.
